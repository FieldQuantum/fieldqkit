...
def Get_Mpo(U, Ns, Ds):
    U = torch.reshape(U, (1, 2**Ns, 1, 2**Ns))

    Mpo = [None] * Ns
    for i in range(0, Ns - 1):
        U = torch.reshape(U, (-1, 2, 2 ** (Ns - i - 1), 1, 2, 2 ** (Ns - i - 1)))
        U = Group(U, [[0, 1, 4], [2, 3, 5]])
        u, s, v = complex_svd(U)
        Dc = min(len(s), Ds)
        Mpo[i] = torch.reshape(u, (-1, 2, 2, Dc)).permute(0, 1, 3, 2) * np.sqrt(2)
...
def Generate_Gate(Ns, depth, train=False):
    gate_all = []
...
def MPS_Circuit(gate_all, Ns, Ds, depth, trun=False):
    MPS = [torch.tensor([1, 0], dtype=torch.cfloat).reshape(1, 2, 1) for i in range(Ns)]
    num = 0
    for d in range(depth):
        for i in range(Ns):
            gate = gate_all[num]

...
| `routing_initial_mapping` | `str` | `"trivial"` | 初始映射策略：`"trivial"`（按顺序映射）或 `"random"`（随机映射）。注意：`"random"` 不支持可沿比特分割的线路（`split_qubits` 返回多组），否则抛出 `ValueError`。 |
| `routing_random_choice` | `bool` | `False` | SABRE 选择 SWAP 时是否随机（而非确定性贪心选最优）。 |
